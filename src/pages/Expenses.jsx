    ...
    <Listbox value={value} onChange={onChange}>
      <div className="relative">
        ...
        <Listbox.Options className="absolute z-50 mt-1 w-full bg-white border border-gray-100 rounded-lg shadow-lg max-h-60 overflow-auto focus:outline-none">
          ...
          {categories.map((cat) => (
            <Listbox.Option key={cat} value={cat} className={({ selected }) => `px-3 py-2 cursor-pointer text-[#64748b] text-sm hover:text-black hover:bg-[#f1f5f9] rounded-lg ${selected ? "bg-[#1e3a5f] text-white" : ""}`}>
              {cat}
            </Listbox.Option>
          ))}
        </Listbox.Options>
  ...
  const columns = [
    ...
    { header: "Date", render: (ex) => formatDate(ex.date, true) || "-" },
    {
      header: "Receipt", render: (ex) => ex.receipt_image ? (
        <a href={ex.receipt_image} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline text-xs">View</a>
      ) : "-"
    },
    {
      header: "Status", render: (ex) => (
        <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold capitalize text-white ${ex.status === "active" ? "bg-emerald-500" : "bg-gray-400"}`}>
          {ex.status}
        </span>
      )
    },
